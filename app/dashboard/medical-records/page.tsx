...
export default async function MedicalRecordsPage({
  ...
  /**
   * This page is the patient self-view.
   *
   * Doctors will use the patient workspace route later.
   */
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
            Review your previous assessments, health metrics, and generated
            medical reports.
          </p>
        </div>
      ...
      <MedicalRecordsHistory
        ...
        paginationBaseHref="/dashboard/medical-records"
        getRecordHref={(record) =>
          `/dashboard/medical-records/${encodeURIComponent(record.id)}`
        }
        allowDelete={true}
      />
    </div>
...
function MedicalRecordRow({ record }: { record: PatientMedicalRecord }) {
  ...
  return (
    <Link
      href={`/dashboard/medical-records/${encodeURIComponent(record.id)}`}
  ...
  return (
    <div className="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
      ...
      <p className="mx-auto mt-1 max-w-md text-xs leading-5 text-slate-500">
        Your completed assessments and medical reports will appear here.
      </p>
    </div>
  );
}
